...
public class Task5 {
    ...
    public static String decrypt(int[] array){
        ...
        char[] result = new char[arrStr.length];
        result[0] = (char)array[0];
        System.arraycopy(array,0,arrStr,0,array.length);

        for(int i = 1; i < arrStr.length; i++){
            result[i] = (char) (arrStr[i-1] + arrStr[i]);
    ...
    public static boolean canMove(String name, String start, String end){
        ...
        int startNum = Integer.parseInt(String.valueOf(start.charAt(1)));

        char endChar = end.charAt(0);
        int endNum = Integer.parseInt(String.valueOf(end.charAt(1)));

        switch (name) {
            case "Qeen": {
                if ((startNum == endNum) && (endChar != startChar) || ((startNum != endNum) && (endChar == startChar)))
                    return true;
                if ((Math.abs(startChar - endChar) == Math.abs(startNum - endNum))) return true;
                break;
            }

            case "Rook": {
                if((startChar == endChar) && (endNum != startNum) || ((startChar != endChar) && (endNum == startNum)))
                    return true;
            }

            case "King": {
                return Math.abs(startNum - endNum) < 2 && Math.abs(startChar - endChar)<2;
            }
            ...
            case"Pawn": {
                if (startChar == endChar && startNum == 2 && endChar ==4)
                    return startChar == endChar && endNum == (startNum+1) ;
            }
    ...
    public static void main(String[] args) throws NoSuchAlgorithmException {
        ...
        System.out.println(Arrays.toString(sameVowelGroup(new String[]{"hoops", "chuff", "bot", "bottom"}).toArray()));
        System.out.println(validateCard(1234567890123452L));
        System.out.println(numToEng(18));
        System.out.println(numToRus(18));
        System.out.println(getSha256Hash("password123"));
        System.out.println(correctTitle("TYRION LANNISTER, HAND OF THE QUEEN."));
        System.out.println(haxLattice(91));
